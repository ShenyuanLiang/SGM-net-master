...
# from src.factorize_graph_matching import (construct_aff_mat, kronecker_sparse,
#                                           kronecker_torch)


from src.lap_solvers.sinkhorn import GumbelSinkhorn, Sinkhorn
...
from src.evaluation_metric import objective_score, objective_score_node, objective_score_edge, Node_distance, \
    Edge_distance
...
class GNNLayer(nn.Module):
    def __init__(self, in_node_features, in_edge_features, out_node_features, out_edge_features,
        ...
        # self.n_self_func2 = nn.Sequential(
        #     nn.Linear(self.in_efeat, self.out_nfeat),
        #     nn.ReLU(),
        #     nn.Linear(self.out_nfeat, self.out_nfeat),
        #     nn.ReLU()
        # )
        # self.combine_func2 = nn.Sequential(
        #     nn.Linear(self.out_nfeat + self.out_efeat, self.out_nfeat + self.out_efeat),
        #     nn.ReLU(),
        #     nn.Linear(self.out_nfeat + self.out_efeat, self.out_nfeat),
        #     nn.ReLU()
        # )
        # self.combine_func3 = nn.Sequential(
        #     nn.Linear(self.out_nfeat + self.out_efeat, self.out_nfeat + self.out_efeat),
        #     nn.ReLU(),
        #     nn.Linear(self.out_nfeat + self.out_efeat, self.out_nfeat),
        #     nn.ReLU()
        # )
        # self.combine_func3_ = nn.Sequential(
        #     nn.Linear(self.out_nfeat + self.out_efeat, self.out_nfeat + self.out_efeat),
        #     nn.ReLU(),
        #     nn.Linear(self.out_nfeat + self.out_efeat, self.out_efeat),
        #     nn.ReLU()
        # )
    ...
    def forward(self, A, W, x, n1=None, n2=None, norm=False):
        ...
        # x2 = W_new1 + self.n_self_func(x)

        x2 = torch.cat((W_new1, self.n_self_func(x)), dim=-1)
        x2 = self.combine_func(x2)

        ## E = A.matmul(W)
        ## E2 = torch.cat((x, E), dim=-1)
        ## x2 = W_new + self.n_self_func(E2)

        # if self.e_func2 is not None:
        #     W1 = A.matmul(W)
        #     # W1 = x
        #     # W2 = torch.cat((A.matmul(W), W1), dim=-1)
        #     W2 = torch.cat((x, W1), dim=-1)
        #     # W2 = W
        #     W_new2 = self.e_func2(W2)
        # else:
        #     W_new2 = W
        #
        # x3 = torch.cat((W_new2, self.n_self_func2(W)), dim=-1)
        # x4 = self.combine_func2(x3)
        ############################################################### update
...
#             conv = SplineConv(input_features, output_features, dim=5, kernel_size=5, aggr="max")
...
class Net(CNN):
    def __init__(self):
        ...
        # self.Graphsage = SAGEConv(1, 30, project=True)

        # self.build_node_aff = pairwise_euclidean_similarity
        # self.build_edge_aff = pairwise_euclidean_similarity
        # self.get_graph_embedding = get_graph_embedding()
        ...
        # )
        #
        # self.combine_func = nn.Linear(2, 1)
    ...
    def forward(self, Kp, Ke, G1, G2, H1, H2, \
        ...
        device = Ke.device
        # Kp[n_points[idx1]:-1, :] = .5
        # node_emb1, _ = self.get_graph_embedding(node_attr1, n_points[0], G1, H1)
        # node_emb2, _ = self.get_graph_embedding(node_attr2, n_points[1], G2, H2)
        # # build_node_aff = Affinity(node_emb1.shape[-1]).to(device)
        # # build_edge_aff = Affinity(edge_emb1.shape[-1]).to(device)
        # Kp = pairwise_cosine_similarity(node_attr1, node_attr2)
        # Ke = pairwise_euclidean_similarity(edge_emb1, edge_emb2)
        # Kp = Kp / Kp.max()
        # Kp = torch.matmul(node_emb1, node_emb2.transpose(1, 2)) / torch.matmul(node_emb1,
        #                                                                        node_emb2.transpose(1, 2)).max()
        # Kp = self.build_node_aff(node_emb1, node_emb2)
        # Ke = self.build_edge_aff(edge_emb1, edge_emb2)
        # Ke = Ke / Ke.max()
        # print(f'Kp = {Kp.max(), Kp.min()}')
        # print(f'Ke = {Ke.max(), Ke.min()}')
        ...
        # emb_edge[n_points[idx1]:-1, :] = emb_edge.mean()

        #   emb_edge1 = torch.matmul(torch.matmul(G1[0].to(device), Ke[0]), G2[0].to(device).T).unsqueeze(-1)
        #   emb_edge2 = torch.matmul(torch.matmul(H1[0].to(device), Ke[0]), H2[0].to(device).T).unsqueeze(-1)
        #   emb_edge = torch.max(self.edge_func(emb_edge1), self.edge_func_(emb_edge2)).squeeze()

        #   emb_edge = self.combine_func(torch.cat((emb_edge1, emb_edge2), dim = -1)).squeeze()
        #############################################################################
        emb_edge = torch.log(emb_edge.view(-1).unsqueeze(-1) + 0.01)
        emb_edge += torch.abs(emb_edge.min())
        emb_edge = (emb_edge / emb_edge.max())
        # emb_edge = emb_edge.view(-1).unsqueeze(-1)
        # emb_edge = self.edge_func_(emb_edge)
        #############################################################################
        ####### Gatconv
        row, col, _ = A.t().coo()
        ...
        # for i in range(self.gnn_layer_edge):
        #     gnn_layer_edge = getattr(self, 'gnn_layer_edge_{}'.format(i))
        #     # gat_layer = getattr(self, 'gat_layer_{}'.format(i))
        #     # emb_node = gat_layer(emb_node, edge_index, edge_emb_)
        #     emb_node2, emb_edge2 = gnn_layer_edge(A2, emb_node2, emb_edge2, n_points[idx1], n_points[idx2])
        ...
        s = (s + s_edge) / 2
        # s = self.combine_func(torch.cat((s[0].unsqueeze(-1),
        #                                  s_edge[0].unsqueeze(-1)), dim=-1)).squeeze().unsqueeze(0)
        # s_edge = v_edge.reshape(G1.shape[-1], G2.shape[-1]).unsqueeze(0)
        # s = (G1.to(device) @ s_edge @ G2.to(device).permute(0, 2, 1)) * \
        #     (H1.to(device) @ s_edge @ H2.to(device).permute(0, 2, 1))
        # s = s + s_edge
        if self.training or cfg.NGM.GUMBEL_SK <= 0:
            ...
            for idx in range(0, gumbel_sample_num, max_repeats):
                ...
                #     objective_score(
                #         ss_gumbel[:, idx:(idx + rep_num), :, :].reshape(-1, ss_gumbel.shape[-2], ss_gumbel.shape[-1]),
                #         repeat(Kp, rep_num), repeat(Ke, rep_num), repeat(G1.to(device), rep_num),
                #         repeat(G2.to(device), rep_num), repeat(H1.to(device), rep_num), repeat(H2.to(device), rep_num)
                #     ).reshape(1, -1)
                # )
                obj_score.append(
...
# model = Net().to(device)
# out = model(Kp, Ke, G1, G2, H1, H2 , n_points)


# %%
# dataset_shape = GMDataset('temp_graph')
#
...
#     n_points = [_.to(device) for _ in
#                 [torch.Tensor([perm_mat.shape[1]]).int(),
#                  torch.Tensor([perm_mat.shape[2]]).int()]]
...
#     Ke = inputs['Ke'].float().to(device)
#     # node_attr1 = inputs['node_embeddings'][0].float().to(device)
#     # node_attr1 = (node_attr1_ - torch.repeat_interleave(
#     #     torch.mean(node_attr1_, dim=2).unsqueeze(-1), n_points[0], dim=-1))
#     # node_attr2 = inputs['node_embeddings'][1].float().to(device)
#     # node_attr2 = (node_attr2_ - torch.repeat_interleave(
#     #     torch.mean(node_attr2_, dim=2).unsqueeze(-1), n_points[1], dim=-1))
#     # edge_attr1 = inputs['edge_attr1'].float().to(device)
#     # edge_attr2 = inputs['edge_attr2'].float().to(device)
...
#     # Ke = torch.Tensor(rbf_kernel(edge1.cpu().numpy()[0], edge2.cpu().numpy()[0],
#     #                              gamma=1 / 1000)).unsqueeze(0).float().to(
#     #     device)
#     # Ke += torch.abs(Ke.min())
#     # A1 = inputs['A1']
#     # A2 = inputs['A2']
#     # G1, H1 = adj2GH(A1[0].numpy())
#     # G2, H2 = adj2GH(A2[0].numpy())
#     # index_null = np.where(~A1[0].numpy().any(axis=0))[0]
#     # Kp[:, index_null, :] = .1
#     # node_emb1, edge_emb1 = get_graph_embedding(edge_attr1, node_attr1, n_points[0], G1, H1)
#     # node_emb2, edge_emb2 = get_graph_embedding(edge_attr2, node_attr2, n_points[1], G2, H2)
#     # # build_node_aff = Affinity(node_emb1.shape[-1]).to(device)
#     # # build_edge_aff = Affinity(edge_emb1.shape[-1]).to(device)
#     # # Kp = build_node_aff(node_emb1, node_emb2)
#     # # Ke = build_edge_aff(edge_emb1, edge_emb2)
#     # Kp = pairwise_euclidean_similarity(node_emb1, node_emb2)
#     # Ke = pairwise_euclidean_similarity(edge_emb1, edge_emb2)
#     # print(f'Kp = {Kp.max(), Kp.min()}')
#     # print(f'Ke = {Ke.max(), Ke.min()}')
...
#
#         Adj_edge = (G1.permute(0, 2, 1).to(device) @ s_pred @ G2.to(device)) * (H1.to(device).permute(0, 2, 1) @ \
#                                                                                 s_pred @ H2.to(device))
#     -1 * objective_score_edge(perm_mat, Kp, Ke, G1.to(device), G2.to(device), H1.to(device), H2.to(device))
...
def eval_model(model, dataloader, visualize=False, dst=''):
    ...
    for inputs in dataloader:
        #  data1, data2 = [_.to(device) for _ in inputs['node_embeddings']]

        #  n1_gt, n2_gt = [_.to(device) for _ in [torch.Tensor([data1.shape[1]]).int(), torch.Tensor([data2.shape[1]]).int()]]
    ...
    print('ACC = {:.4f} ({}/{}) in {:.2f} seconds'.format((acc_match_num / acc_total_num).item(),
                                                          ...
                                                          time.time() - since))
    # return obj/len(dataloader.dataset)
...
def train_eval_model(model, dataloader_train, dataloader_test):
    ...
    for epoch in range(start_epoch, num_epochs):
        # if epoch == 40 - 1:
        # optimizer = optim.SGD(model.parameters(), lr=1.0e-4, momentum=0.9, nesterov=True)
        # scheduler = get_scheduler(optimizer)
        # print('Start using SGD')
        ...
        for inputs in dataloader_train:
            ...
            # node_attr1 = inputs['node_embeddings'][0].float().to(device)
            # node_attr1 = (node_attr1_ - torch.repeat_interleave(
            #     torch.mean(node_attr1_, dim=2).unsqueeze(-1), n_points[0], dim=-1)) / node_attr1_.max()
            # node_attr2 = inputs['node_embeddings'][1].float().to(device)
            # node_attr2 = (node_attr2_ - torch.repeat_interleave(
            #     torch.mean(node_attr2_, dim=2).unsqueeze(-1), n_points[1], dim=-1)) / node_attr2_.max()
            # edge_attr1 = inputs['edge_attr1'].float().to(device)
            # edge_attr2 = inputs['edge_attr2'].float().to(device)
            G1, G2 = [_ for _ in inputs['G']]
            H1, H2 = [_ for _ in inputs['H']]
...
# 'new_code/graph_test/dataset/**'  Train path sample
# 'new_code/graph_test/dataset_test' Test path sample
if args.Train:
...
if args.Inference:
    ...
    for inputs in tqdm(test_loader):
        ...
        # edge_attr2 = inputs['edge_attr2'].float().to(device)
        Kp = inputs['Kp'].float().to(device)
        # Kp = torch.nn.functional.normalize(Kp)
        Ke = inputs['Ke'].float().to(device) * args.weights_on_edges
        # Ke = torch.nn.functional.normalize(Ke)
        # Ke = Ke/Ke.max()
        G1, G2 = [_ for _ in inputs['G']]
        H1, H2 = [_ for _ in inputs['H']]
        # A1 = inputs['A1']
    ...
    # print(f'node distance reduction [Network] is: {(node_dis_red / len(test_loader)).item()}')
    # print(f'edge distance reduction [Network] is: {(edge_dis_red / len(test_loader)).item()}')
    # print(f'node distance reduction [Matlab] is: {(node_dis_red_mat / len(test_loader)).item()}')
    # print(f'edge distance reduction [Matlab] is: {(edge_dis_red_mat / len(test_loader)).item()}')
...
# plt.subplot(122)
# plt.plot(acc_all)
# plt.title('testing objective ratio', fontdict = {'fontsize' : 12})
# plt.savefig("/home/EG-matching/sim_true2.pdf", dpi = 300)
