...
def objective_score(pmat_pred: Tensor, Kp: Tensor, Ke: Tensor, G1: Tensor,
                    G2: Tensor, H1: Tensor, H2: Tensor) -> Tensor:
    r"""
    Objective score given predicted permutation matrix and affinity matrix from the problem.
    ...
    """
    batch_num = pmat_pred.shape[0]
    obj1 = torch.matmul(Kp.permute(0, 2, 1), pmat_pred)
    # U, S, V = torch.svd(Ke)
    # U = torch.matmul(U, torch.diag_embed(S, offset=0, dim1=-2, dim2=-1))
    # A1 = G1 @ torch.diag_embed(U, offset=0, dim1=-2, dim2=-1) @ H1.permute(0, 2, 1)
    # A2 = G2 @ torch.diag_embed(V, offset=0, dim1=-2, dim2=-1) @ H2.permute(0, 2, 1)
    obj2 = Ke.permute(0, 2, 1) @ (torch.max((G1.permute(0, 2, 1) @ pmat_pred @ G2), (H1.permute(0, 2, 1) @ \
                                                                                     pmat_pred @ H2)))
    # obj_score = obj1.diagonal(offset=0, dim1=-1, dim2=-2).sum(-1) + obj2.diagonal(offset=0, dim1=-1, dim2=-2).sum(
    #     -1)
    obj_score = obj2.diagonal(offset=0, dim1=-1, dim2=-2).sum(-1)

    return -1 * obj_score


def objective_score_node(pmat_pred: Tensor, Kp: Tensor, Ke: Tensor, G1: Tensor,
                         G2: Tensor, H1: Tensor, H2: Tensor) -> Tensor:
    r"""
    Objective score given predicted permutation matrix and affinity matrix from the problem.
    ...
    batch_num = pmat_pred.shape[0]
    obj1 = torch.matmul(Kp.permute(0, 2, 1), pmat_pred)
    # obj2 = Ke.permute(0, 2, 1) @ ((G1.permute(0, 2, 1) @ pmat_pred @ G2) * (H1.permute(0, 2, 1) @ \
    #                                                                         pmat_pred @ H2))
    obj_score = obj1.diagonal(offset=0, dim1=-1, dim2=-2).sum(-1)
    # obj_score = obj2.diagonal(offset=0, dim1=-1, dim2=-2).sum(-1)

    return -1 * obj_score


def objective_score_edge(pmat_pred: Tensor, Kp: Tensor, Ke: Tensor, G1: Tensor,
                         G2: Tensor, H1: Tensor, H2: Tensor) -> Tensor:
    r"""
    Objective score given predicted permutation matrix and affinity matrix from the problem.
    ...
    obj2 = Ke.permute(0, 2, 1) @ ((G1.permute(0, 2, 1) @ pmat_pred @ G2) * (H1.permute(0, 2, 1) @ \
                                                                            pmat_pred @ H2))
    # obj2 = Ke.permute(0, 2, 1) @ (G1.permute(0, 2, 1) @ pmat_pred @ G2)
    # obj_score = obj1.diagonal(offset=0, dim1=-1, dim2=-2).sum(-1)
    obj_score = obj2.diagonal(offset=0, dim1=-1, dim2=-2).sum(-1)
...
def Edge_distance(pmat_pred: Tensor, Kp: Tensor, Ke: Tensor, G1: Tensor,
                  G2: Tensor, H1: Tensor, H2: Tensor) -> Tensor:
    batch_num = pmat_pred.shape[0]
    pmat_org = torch.eye(pmat_pred.shape[1], pmat_pred.shape[2]).unsqueeze(0)
    # obj1 = torch.matmul(Kp.permute(0, 2, 1), pmat_pred)
    # obj2 = Ke.permute(0, 2, 1) @ ((G1.permute(0, 2, 1) @ pmat_pred @ G2) * (H1.permute(0, 2, 1) @ \
    #                                                                         pmat_pred @ H2))
    # obj_score = obj1.diagonal(offset=0, dim1=-1, dim2=-2).sum(-1)
    dis = 6 * (.5 - Ke.permute(0, 2, 1)) @ (G1.permute(0, 2, 1) @ pmat_pred @ G2)
    dis_org = 6 * (.5 - Ke.permute(0, 2, 1)) @ (G1.permute(0, 2, 1) @ pmat_org @ G2)

    return 1 - dis.diagonal(offset=0, dim1=-1, dim2=-2).sum(-1) / dis_org.diagonal(offset=0, dim1=-1, dim2=-2).sum(-1)


def Node_distance(pmat_pred: Tensor, Kp: Tensor, Ke: Tensor, G1: Tensor,
                  G2: Tensor, H1: Tensor, H2: Tensor) -> Tensor:
    batch_num = pmat_pred.shape[0]
    pmat_org = torch.eye(pmat_pred.shape[1], pmat_pred.shape[2]).unsqueeze(0)
    # obj1 = torch.matmul(Kp.permute(0, 2, 1), pmat_pred)
    ...
    #                                                                         pmat_pred @ H2))
    # obj_score = obj1.diagonal(offset=0, dim1=-1, dim2=-2).sum(-1)
    dis = 500 * torch.matmul(1 - Kp.permute(0, 2, 1), pmat_pred)
    dis_org = 500 * torch.matmul(1 - Kp.permute(0, 2, 1), pmat_org)
...
def clustering_purity(pred_clusters: Tensor, gt_classes: Tensor) -> Tensor:
    ...
    for b in range(batch_num):
        for i in range(num_clusters[b]):
            max_counts = torch.max(torch.unique(gt_clusters[b][pred_clusters[b] == i], return_counts=True)[-1]).to(
                dtype=torch.float)
