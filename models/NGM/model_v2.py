...
import itertools
import sys
sys.path.append('/home/EG-matching')
...
class Net(CNN):
    ...
    def forward(
        ...
        # print(f'nodes: {orig_graph_list[0][0].x.shape}, node_weight:{global_weights_list[0].shape}')
        # print(f'edges: {orig_graph_list[0][0].edge_attr.shape}, edge_weight:{global_weights_list[0]}')
        unary_affs_list = [
        ...
        for unary_affs, quadratic_affs, (idx1, idx2) in zip(unary_affs_list, quadratic_affs_list, lexico_iter(range(num_graphs))):
            ...
            print(f' Ke is {[Ke.min().item(), Ke.max().item()]}, Kp:{[Kp.min().item(), Kp.max().item()]}')
            K = construct_aff_mat(Ke, Kp, kro_G, kro_H)
            print(f'max K: {K.max().item()}, min K: {K.min().item()}')
            ...
            # s = v.view(v.shape[0], n_points[idx2][0], -1).transpose(1, 2)

            ss = self.sinkhorn(s, n_points[idx1], n_points[idx2], dummy_row=True)
            x = hungarian(ss, n_points[idx1], n_points[idx2])
            # print(f'npoints1 is {n_points[idx1], idx1}, npoints2 is {n_points[idx2], idx2}')
            # print(f'points[idx2]: {points[idx2].shape[1]}')

            s_list.append(ss)
            x_list.append(x)
            indices.append((idx1, idx2))
        # print(f'The shape of Ke is {[Ke.min(), Ke.max()]}, Kp:{[Kp.min(), Kp.max()]}')
        if num_graphs > 2:
