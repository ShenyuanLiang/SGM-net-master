...
class GraphPairDataset:
    ...
    def __load_MATLAB(filename):
        ...
        # get 1st graph
        G1_node_embeddings = np.asarray(graph_pair['pair']['G1'][0][0]['nodeXY'][0][0], dtype=np.float32).T / 100.0
            ...
            for j in range(i + 1, G1_num_nodes):
                if G1_adjacency_matrix[i, j] > 0:
                    for ii, jj in [[i, j], [j, i]]:
                        ...
                        # G1_edge_embeddings.append(np.expand_dims(np.asarray(pts.max()), -1))

                        """
                        #visualize
                        pts*=100.0
                        minp = np.amin(pts, axis=0)
                        maxp = np.amax(pts, axis=0)
                        width = int(maxp[0]-minp[0])+10
                        height = int(maxp[1]-minp[1])+10
                        img = np.zeros((height,width,3), np.uint8)
                        img = cv2.line(img, (0, int(-minp[1])+5), (width-1, int(-minp[1])+5), (255,0,255), 1)
                        img = cv2.line(img, (int(-minp[0])+5, 0), (int(-minp[0])+5, height-1), (255,0,255), 1)
                        for k in range(30):
                          if k==0:
                            color=(0,255,0)
                          elif k==29:
                            ...
                            color=(255,255,255)
                          img = cv2.circle(img, (int(pts[k,0]-minp[0])+5, int(pts[k,1]-minp[1])+5), 1, color, -1)
                        print(ii, jj)
                        cv2_imshow(img)
                        """

        # get 2nd graph and find null nodes
        G2_node_embeddings = np.asarray(graph_pair['pair']['G2'][0][0]['nodeXY'][0][0], dtype=np.float32).T / 100.0
            ...
            for j in range(i + 1, G2_num_nodes):
                if G2_adjacency_matrix[i, j] > 0:
                    for ii, jj in [[i, j], [j, i]]:
                        ...
                        # G2_edge_embeddings.append(np.expand_dims(np.asarray(pts.max()), -1))
        ...
        # delete null nodes from the 2nd graph
        """
        G2_node_embeddings = G2_node_embeddings[valid_nodes]
    
        G2_num_nodes = sum(valid_nodes)
    
        new_indices = []
        j = 0
        for flag in valid_nodes:
          if flag:
        ...
        for i in range(len(G2_adjacency_list)):
          j, k = G2_adjacency_list[i]
          G2_adjacency_list[i] = [new_indices[j], new_indices[k]]
        """
        ...
        #       {'num_nodes': G1_num_nodes, 'node_embeddings': G1_node_embeddings, 'num_edges': G1_num_edges, 'adjacency_list': G1_adjacency_list, 'edge_embeddings': G1_edge_embeddings}, \
        #      {'num_nodes': G2_num_nodes, 'node_embeddings': G2_node_embeddings, 'num_edges': G2_num_edges, 'adjacency_list': G2_adjacency_list, 'edge_embeddings': G2_edge_embeddings}
        return permutation_vector, \
               {'num_nodes': G1_num_nodes, 'node_embeddings': G1_node_embeddings, 'num_edges': G1_num_edges,
                'adjacency_list': G1_adjacency_list, 'edge_embeddings': G1_edge_embeddings, 'G_edgeaff': G_edgeaff,
                'G_nodeaff': G_nodeaff, 'adj': G1_adjacency_matrix}, \
               {'num_nodes': G2_num_nodes, 'node_embeddings': G2_node_embeddings, 'num_edges': G2_num_edges,
                'adjacency_list': G2_adjacency_list, 'edge_embeddings': G2_edge_embeddings, 'adj': G2_adjacency_matrix}

    # load graph file
    @staticmethod
    def load_pair(filename):
        # assert cfg.DATASET.TYPE == 'MATLAB', 'Dataset type "{}" is invalid, check configuration file'.format(cfg.DATASET.TYPE)
...
class GMDataset(torch.utils.data.Dataset):
    def __init__(self, path: str, mode='train'):
    ...
    def __getitem__(self, idx):
        ...
        for G in [pair['G1'], pair['G2']]:
            adjacency_matrix = np.zeros((G['num_nodes'] + G['num_edges'], G['num_nodes'] + G['num_edges']), np.float32)
        ...
        permutation_matrix = np.zeros(
            (pair['G1']['num_nodes'] + pair['G1']['num_edges'], pair['G2']['num_nodes'] + pair['G2']['num_edges']),
            np.int32)
        for i, j in enumerate(pair['permutation_vector']):
            permutation_matrix[i, j] = 1
        edge_indices = {(u, v): i for i, (u, v) in enumerate(pair['G2']['adjacency_list'])}
        for i, (u, v) in enumerate(pair['G1']['adjacency_list']):
            key = (pair['permutation_vector'][u], pair['permutation_vector'][v])
            if key in edge_indices:
                j = edge_indices[key]
                permutation_matrix[pair['G1']['num_nodes'] + i, pair['G2']['num_nodes'] + j] = 1
        ...
        ret_dict = {
            ...
            'num_edges': [pair['G1']['num_edges'], pair['G2']['num_edges']],
            'edge_embeddings': [torch.from_numpy(x).float() for x in
                                [pair['G1']['edge_embeddings'], pair['G2']['edge_embeddings']]],
            'adjacency_matrices': adjacency_matrices,
            'A1': torch.from_numpy(pair['G1']['adj']),
            'A2': torch.from_numpy(pair['G2']['adj'])
        }
