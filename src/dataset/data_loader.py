...
class GMDataset(Dataset):
    ...
    def to_pyg_graph(A, P):
        ...
        # print(f'A1 shape is {pyg_graph.edge_attr.shape}, type is {type(pyg_graph.edge_attr)}')
        return pyg_graph
    ...
    def get_pair(self, idx, cls):
        ...
        # print(f'A1 shape is {A1.shape}, type is {type(A1)}')
...
def collate_fn(data: list):
    ...
    if 'Gs' in ret and 'Hs' in ret:
        if cfg.PROBLEM.TYPE == '2GM' and len(ret['Gs']) == 2 and len(ret['Hs']) == 2:
            G1, G2 = ret['Gs']
            # print(f'G1:{type(G1)}, length of G1:{G1.shape}')
            H1, H2 = ret['Hs']
            if cfg.FP16:
            ...
            # print(f'length Kro_G1:{len(K1G)}, Kro_G2:{K1G[1].shape}, Kro_G type:{type(K1G)}')
            K1H = [kronecker_sparse(x, y).astype(sparse_dtype) for x, y in zip(H2, H1)]
            K1G = CSRMatrix3d(K1G)
            K1H = CSRMatrix3d(K1H).transpose()
            # print(f'Kro_G:{K1G.shape}, Kro_H:{K1H.shape}')
            ret['KGHs'] = K1G, K1H
            ...
            for idx_1, idx_2 in product(range(len(ret['Gs'])), repeat=2):
                ...
                # print(f'Kro_G:{KG.shape}, Kro_H:{KH.shape}')
